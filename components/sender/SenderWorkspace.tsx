...
import { ChevronLeft, PlaneTakeoff, Box, Coins, Trash2 } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        <Link href="/senders" className="text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-50">
          <ChevronLeft className="h-6 w-6" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">{senderName} Workspace</h1>
          <p className="text-zinc-500 dark:text-zinc-400 mt-2">Manage shipments and boxes for this sender.</p>
        </div>
      </div>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead className="text-xs text-zinc-500 uppercase bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
                    <tr>
                      <th className="px-6 py-3">Reference Box</th>
                      ...
                      paginatedShipments.map((s) => {
                        ...
                        return (
                          <tr key={s.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-zinc-500 truncate max-w-[200px]" title={trkNumbers || undefined}>
                              {trkNumbers || '—'}
                            </td>
                            <td className="px-6 py-4 text-zinc-700 dark:text-zinc-300">{s.country}</td>
                            <td className="px-6 py-4 text-zinc-700 dark:text-zinc-300">{s.recipient_name}</td>
                      ...
                      <div key={s.id} className="p-4 space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs pt-1 border-t border-zinc-100 dark:border-zinc-900">
                          ...
                          <div>
                            ...
                            <span className="font-medium text-zinc-850 dark:text-zinc-200">{s.recipient_name}</span>
                          </div>
                          <div>
                    ...
                    {paginatedBoxes.length > 0 ? (
                      paginatedBoxes.map((b) => (
                        <tr key={b.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-zinc-500 dark:text-zinc-400 font-mono text-xs">{b.tracking_number || '—'}</td>
                          <td className="px-6 py-4 text-zinc-600 dark:text-zinc-400">{b.weight != null ? `${b.weight} kg` : '—'}</td>
                          <td className="px-6 py-4 text-zinc-700 dark:text-zinc-300 max-w-[200px] truncate">{b.content}</td>
                      ...
                      <tr>
                        <td colSpan={7} className="px-6 py-8 text-center text-zinc-500">
                          No boxes found for this sender.
                        </td>
                      </tr>
                    )}
                  </tbody>
